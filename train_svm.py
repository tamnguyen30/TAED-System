...
pipeline = Pipeline([
    ...
    ('svm', LinearSVC(random_state=42, dual=True)) # dual=True is a safe default
])
...
# Note: LinearSVC does not have predict_proba by default. We'll get ROC-AUC from decision_function.
try:
    y_scores = pipeline.decision_function(X_test)
except AttributeError:
    y_scores = y_pred # Fallback if decision_function is not available
end_time = time.time()
