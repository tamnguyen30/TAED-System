...
model = TFDistilBertForSequenceClassification.from_pretrained(MODEL_CHECKPOINT, num_labels=2, use_safetensors=False)
optimizer = tf.keras.optimizers.Adam(learning_rate=5e-5)
model.compile(optimizer=optimizer, loss=model.hf_compute_loss, metrics=['accuracy'])

# 5. Fine-tune the model
print(f"\nFine-tuning the model for {EPOCHS} epoch(s)... (This will take a long time!)")
...
print(f"5. Latency:  {latency:.4f} seconds for {len(y_test)} predictions")
